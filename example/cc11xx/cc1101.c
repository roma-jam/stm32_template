...
static inline uint8_t cc1101_read_register(CC1101* cc1101, uint8_t addr)
{
    ...
    {
#if (CC1101_DEBUG_ERRORS)
        printf("CC1101:read register error\n");
#endif // CC1101_DEBUG_ERRORS
        cc1101_cs_hi();
        return 0;
    }
...
static inline bool cc1101_write_strobe(CC1101* cc1101, uint8_t strobe)
{
    *(uint8_t*)io_data(cc1101->io) = strobe;
    cc1101->io->data_size = sizeof(uint8_t);

    cc1101_cs_lo();
    while(cc1101_busy());
    if(sizeof(uint8_t) != spi_data(CC1101_SPI, cc1101->io, CC1101_IO_SIZE))
    {
#if (CC1101_DEBUG_ERRORS)
        printf("CC1101: write strobe error\n");
#endif // CC1101_DEBUG_ERRORS
        cc1101_cs_hi();
        return false;
    }
    ...
    cc1101->state = *(uint8_t*)io_data(cc1101->io);
...
static inline bool cc1101_chip_reset(CC1101* cc1101)
{
    return cc1101_write_strobe(cc1101, CC_SRES);
}
...
static inline bool cc1101_flush_tx_fifo(CC1101* cc1101)
{
    return cc1101_write_strobe(cc1101, CC_SFTX);
}

static inline bool cc1101_flush_rx_fifo(CC1101* cc1101)
{
    return cc1101_write_strobe(cc1101, CC_SFRX);
}
...
static inline bool cc1101_rf_config(CC1101* cc1101)
{
...
{
    uint8_t partnum = cc1101_read_register(cc1101, CC1101_PARTNUM);
    uint8_t version = cc1101_read_register(cc1101, CC1101_VERSION);
    uint8_t RSSI = cc1101_read_register(cc1101, CC1101_RSSI);
...
//    printf("CC1101: init\n");
    ...
    pin_enable(CC1101_GDO0_PIN, STM32_GPIO_MODE_INPUT_PULL, false);
    pin_enable(CC1101_GDO2_PIN, STM32_GPIO_MODE_INPUT_PULL, false);
    ...
    if(!cc1101_chip_reset(cc1101))
    {
#if (CC1101_DEBUG_ERRORS)
        printf("CC1101: reset failure\n");
#endif // CC1101_DEBUG_ERRORS
        return;
    }

    if(!cc1101_flush_rx_fifo(cc1101))
    {
#if (CC1101_DEBUG_ERRORS)
